    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-xl rounded-2xl border border-zinc-700 bg-zinc-900 p-6">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <Link to="/auth?role=developer" onClick={onClose} className="rounded-xl bg-orange-500 px-4 py-3 text-center font-semibold text-black">Join as Developer</Link>
          <Link to="/auth?role=player" onClick={onClose} className="rounded-xl border border-zinc-700 px-4 py-3 text-center hover:border-orange-300">Join as Player</Link>
        </div>
